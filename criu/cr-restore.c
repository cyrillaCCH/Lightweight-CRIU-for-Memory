...
static int restore_vma_settings(struct parasite_ctl *ctl, struct mem_rst_args *arg, MmEntry *mm, int num_lines, void *addr[][2]) {
	...
	for (; i < mm->n_vmas; i++) {
		arg->vmas[j] = *mm->vmas[i];
		strncpy(arg->path[j], arg->vmas[j].name, PATH_LEN - 1);
		j++;
...
static int restore_memory(pid_t pid)
{
	...
	{
		uint64_t time_us = timing_total(TIME_MEMRST);
		...
		pr_msg("Restore pages total time: %" PRIu64 " us, size: %" PRIu64 " bytes, throughput: %.2f MB/s\n",
